...
class DatabaseSQLRepository:
    ...
    async def _set_order_by(self, stmt: Select, sort: DatabasePostgresSort) -> Select:
        if self._model is None:
            raise ValueError('Model is not set')
        ...
        for column, order in columns_and_orders.items():
            if column in self._model.__table__.columns:
                if order == str(DatabasePostgresSort.DESC):
                    stmt = stmt.order_by(desc(column))
                elif order == str(DatabasePostgresSort.ASC):
